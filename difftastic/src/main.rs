...
fn diff_file_content(
    ...
    if diff_options.get_addtions {
        for hunk in hunks.iter() {
            // std::println!("novel_lhs for this Hunk:");
            for &line_number in hunk.novel_rhs.iter() {
                std::println!("{:?}", line_number.display());
            }
        }
        std::process::exit(EXIT_SUCCESS);
    } else if diff_options.get_deletions {
        for hunk in hunks.iter() {
            // std::println!("novel_lhs for this Hunk:");
            for &line_number in hunk.novel_lhs.iter() {
                std::println!("{:?}", line_number.display());
            }
        }
        std::process::exit(EXIT_SUCCESS);
    } else if diff_options.get_line_map {
        // 输出 DftHashMap
        for (key, value) in &opposite_to_rhs {
